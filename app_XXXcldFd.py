...
def fetch_available_tests():
    ...
    try:
        response = requests.get(AVAILABLE_TESTS_URL, timeout=10)  # Reduced timeout
...
# Load available tests if not already loaded - DISABLED to prevent startup timeout
# if not st.session_state.available_tests:
#     enhanced_tests, basic_tests = fetch_available_tests()
#     st.session_state.available_tests = enhanced_tests

# Fallback to default tests to prevent startup issues
if not st.session_state.available_tests:
    st.session_state.available_tests = [{
...
if st.session_state.study_analysis:
    ...
    with col1:
        ...
        if suggested_test == "survival_analysis":
            st.error(f"**🎯 Recommended Test:** {get_test_display_name(suggested_test)}")
            ...
            st.success(f"**🎯 Recommended Test:** {get_test_display_name(suggested_test)}")
        
        # Compact study details with alternative test icon
        data_type = analysis.get('data_type', 'Unknown')
        study_design = analysis.get('study_design', 'Unknown')
        
        detail_col1, detail_col2, detail_col3 = st.columns([1, 2, 0.3])
        with detail_col1:
            st.write(f"**Data Type:** {data_type.title()}")
        with detail_col2:
            st.write(f"**Study Design:** {study_design.replace('_', ' ').title()}")
        with detail_col3:
            # Alternative tests as compact icon
            alternatives = analysis.get('alternative_tests', [])
            if alternatives:
                with st.popover("🔄", help="Alternative test options"):
                    st.write("**Other suitable tests:**")
    ...
    if parameters:
        ...
        if 'total_n' in parameters:
            param_items.append(f"**N:** {parameters['total_n']}")
        if 'alpha' in parameters:
            param_items.append(f"**α:** {parameters['alpha']}")
        if 'power' in parameters:
            param_items.append(f"**Power:** {parameters['power']*100:.0f}%")
        ...
        if effect_size_val:
            ...
            param_items.append(f"**Effect:** {effect_size_val:.3f}")
        
        if param_items:
            st.caption("📊 " + " | ".join(param_items))
...
# Only show after analysis is run
if st.session_state.study_analysis:
    ...
    # Additional AI analysis sections (only after analysis)
    ...
    if st.session_state.llm_provider_used:
...
# === Dynamic Parameter Forms (Only After Analysis) ===
if st.session_state.study_analysis:
    ...
    with col1:
        ...
        if st.session_state.p_value_message:
            ...
            if p_val == 0:
                ...
                p_display = f"{p_val:.2e}"  # Use scientific for very small values
                p_sci = f"{p_val:.2e}"
            elif p_val < 0.001:
                p_display = f"{p_val:.4f}"  # Use decimal as default
                p_sci = f"{p_val:.2e}"
            elif p_val < 0.01:
                p_display = f"{p_val:.4f}"  # Use decimal as default
                p_sci = f"{p_val:.2e}"
            else:
                p_display = f"{p_val:.3f}"  # Use 3 decimals for larger p-values
                p_sci = f"{p_val:.2e}"
